...
def truncateBitString(inverted_new_sum, numBytes):
    ...
    if (len(inverted_new_sum) < 8 * numBytes):
        while (len(inverted_new_sum) < 8 * numBytes):
            inverted_new_sum = "0" + inverted_new_sum
    print(inverted_new_sum)
    return inverted_new_sum

def calculateChecksumWithoutInverting(binaryString1, binaryString2):
