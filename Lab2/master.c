...
char *bytesToIPString(char arr[4])
{
	char *IP;
...
// void IPStringToBytes(char *IP, char bytes[4])
// {
// 	char *c;
// 	int index = 0;
...
void addSlaveNode(struct Node *master, struct Node *slave)
{
	...
	for (int i = 0; i < 4; i++)
	{
		slave->nextSlaveIP[i] = master->next->IP[i];
	}
	slave->next = master->next;
	for (int i = 0; i < 4; i++)
	{
		master->nextSlaveIP[i] = slave->IP[i];
	}
...
int main(int argc, char *argv[])
{
	...
	char* thing = host_entry->h_addr_list[0];
	printf("SOMETHING: %s", thing);
	// for(int i = 0; i < strlen(thing); i++) {
	// 	printf("Byte: %c\n", thing[i]);
	// }
	// for(int i = 0; i < strlen(host_entry->h_addr_list[0]))
	...
	for (int i = 0; i < 4; i++)
	{
		master->nextSlaveIP[i] = master->next->IP[i];
	}
	...
	{ // main accept() loop
		...
		for (int i = 0; i < 4; i++)
		{
			response.nextSlaveIP[i] = slave->nextSlaveIP[i];
		}
